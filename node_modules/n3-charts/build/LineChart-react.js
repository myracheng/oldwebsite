...
(function (n3Charts) {
    ...
    (function (Factory) {
        'use strict';
        var ReactSyncLayer = (function (_super) {
            __extends(ReactSyncLayer, _super);
            function ReactSyncLayer(properties) {
                _super.call(this);
                this.properties = properties;
            }
            ReactSyncLayer.prototype.create = function () {
            ...
            ReactSyncLayer.prototype.sanitizeAttributes = function () {
                var _a = this.properties, tooltipSyncKey = _a.tooltipSyncKey, domainsSyncKey = _a.domainsSyncKey;
            ...
            ReactSyncLayer.prototype.syncDatumEvents = function () {
                var _this = this;
                var eventMgr = this.eventMgr;
                if (!!this.properties.onClick) {
                    eventMgr.on('click.sync-layer', function (d, i, series, options) {
                        _this.properties.onClick(d, i, series, options);
                    });
                }
            };
            ReactSyncLayer.prototype.outerWorldHover = function (value) {
                this.eventMgr.triggerDataAndOptions('outer-world-hover', value);
            };
            ReactSyncLayer.prototype.syncTooltips = function () {
                var _this = this;
                if (!!this.properties.tooltipSyncKey) {
                    ...
                    this.eventMgr.on('container-move.sync-layer', function (event) {
                        layers.forEach(function (layer) {
                            if (layer !== _this) {
                                layer.outerWorldHover(_this.factoryMgr.get('container').getCoordinatesFromEvent(event));
                            }
                        });
                    });
                    this.eventMgr.on('container-out.sync-layer', function () {
                        layers.forEach(function (layer) {
                            if (layer !== _this) {
            ...
            ReactSyncLayer.prototype.outerWorldSync = function (domains, type) {
                var xAxis = this.factoryMgr.get('x-axis');
                var x2Axis = this.factoryMgr.get('x2-axis');
                var yAxis = this.factoryMgr.get('y-axis');
                var y2Axis = this.factoryMgr.get('y2-axis');
            ...
            ReactSyncLayer.prototype.syncDomainsChange = function () {
                var _this = this;
                var callbacks = [];
                var xAxis = this.factoryMgr.get('x-axis');
                var yAxis = this.factoryMgr.get('y-axis');
                if (!!this.properties.onDomainsChange) {
                    callbacks.push(function (domains, _a) {
                        ...
                        if (isEndEvent) {
                            _this.properties.onDomainsChange(domains);
                        }
                    });
                }
                ...
                this.eventMgr.on('pan.sync-layer', function () {
                    var domains = getDomains();
                    _this.factoryMgr.get('pan').constrainDomains(domains);
                    ping(domains, { type: 'pan' });
                });
                this.eventMgr.on('pan-end.sync-layer', function () {
                    var domains = getDomains();
                    _this.factoryMgr.get('pan').constrainDomains(domains);
                    ping(domains, { type: 'pan-end', isEndEvent: true });
                });
                this.eventMgr.on('zoom.sync-layer', function () {
                    var domains = getDomains();
                    _this.factoryMgr.get('zoom').constrainOutgoingDomains(domains);
                    ping(domains, { type: 'zoom', isEndEvent: false });
                });
                this.eventMgr.on('zoom-end.sync-layer', function () {
                    var domains = getDomains();
                    _this.factoryMgr.get('zoom').constrainOutgoingDomains(domains);
                    ping(domains, { type: 'zoom-end', isEndEvent: true });
                });
                this.eventMgr.on('zoom-pan-reset.sync-layer', function (madeHere) {
                    if (madeHere) {
                        ping(getDomains(), { type: 'zoom-pan-reset', isEndEvent: true });
                    }
                });
            };
            ReactSyncLayer.prototype.destroy = function () {
                var fn;
                while (fn = this.unregisteringFunctions.pop()) {
                    fn();
                }
            };
            ReactSyncLayer.tooltipSyncLayers = {};
            ReactSyncLayer.domainSyncLayers = {};
            return ReactSyncLayer;
        }(Factory.BaseFactory));
        Factory.ReactSyncLayer = ReactSyncLayer;
    })(Factory = n3Charts.Factory || (n3Charts.Factory = {}));
})(n3Charts || (n3Charts = {}));
...
var LineChart = React.createClass({
    ...
    componentDidMount: function () {
        ...
        this.factoryMgr.all().forEach(function (f) { return f.instance.init(f.key, _this.eventMgr, _this.factoryMgr); });
        ...
        this.eventMgr.on('legend-click.directive', function (series) {
            var foundSeries = _this.options.series.filter(function (s) { return s.id === series.id; })[0];
            foundSeries.visible = series.getToggledVisibility();
            _this.eventMgr.trigger('update', _this.data, _this.options);
        });
        this.eventMgr.on('pan.directive', function () {
            _this.factoryMgr.get('container').svg.classed('panning', true);
        });
        this.eventMgr.on('pan-end.directive', function () {
            _this.factoryMgr.get('container').svg.classed('panning', false);
        });
    },
